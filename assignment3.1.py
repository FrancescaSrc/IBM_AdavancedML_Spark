...
def minTemperature(df,spark):
    ...
    mintemp= spark.sql("SELECT min(temperature) as mintemp FROM parquetFile").first().mintemp
    return mintemp
...
def meanTemperature(df,spark):
    df.createOrReplaceTempView("parquetFile")
    return spark.sql("SELECT avg(temperature) as avg FROM parquetFile").first().avg
...
def maxTemperature(df,spark):
    df.createOrReplaceTempView("parquetFile")
    return spark.sql("SELECT max(temperature) as max FROM parquetFile where temperature is not null").first().max
...
def sdTemperature(df,spark):
    df.createOrReplaceTempView("parquetFile")
    return spark.sql("SELECT std(temperature) as std FROM parquetFile where temperature is not null").first().std
...
as skewt from parquetFile
                    """ %(meanTemperature(df,spark),sdTemperature(df,spark))).first().skewt
...
def correlationTemperatureHardness(df,spark):
    df.createOrReplaceTempView("parquetFile")
    return spark.sql("SELECT corr(temperature, hardness) as corr FROM parquetFile").first().corr
